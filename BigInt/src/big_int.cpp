...
BigInt::BigInt(const std::string &num) {
	...
	for(int sectionEnd = static_cast<int>(num.size()), sectionStart = std::max(sectionEnd - BigInt::digitsPerBucket, start); sectionEnd != start; sectionEnd = sectionStart, sectionStart = std::max(sectionEnd - BigInt::digitsPerBucket, start)) {
		this->digits.push_back(std::stoi(num.substr(static_cast<size_t>(sectionStart), static_cast<size_t>(sectionEnd - sectionStart))));
	}
...
BigInt::BigInt(const long long num) {
	...
	// can't do abs on num: abs doesn't return a positive number of LONG_LONG_MIN
...
std::ostream& operator<<(std::ostream &out, const BigInt &integer) {
	...
	for(std::vector<int>::const_reverse_iterator it = integer.digits.crbegin(); it != integer.digits.crend(); ++it) {
		int bucket = *it;
...
BigInt BigInt::halfAdd(const BigInt &left, const BigInt &right) {
	...
	int carry = 0;
	for(size_t i = 0; i < std::max(left.digits.size(), right.digits.size()); ++i) {
...
BigInt BigInt::multiplyNaive(const BigInt &left, const BigInt &right) {
	...
	std::vector<long long> product(left.digits.size() + right.digits.size() + 2);
...
BigInt BigInt::multiplyKaratsubaHelper(const BigInt &left, const BigInt &right) {
	...
	BigInt A(false, std::vector<int>(left.digits.begin() + splitPoint, left.digits.end()));
	BigInt B(false, std::vector<int>(left.digits.begin(), left.digits.begin() + splitPoint));
	BigInt C(false, std::vector<int>(right.digits.begin() + splitPoint, right.digits.end()));
	BigInt D(false, std::vector<int>(right.digits.begin(), right.digits.begin() + splitPoint));
