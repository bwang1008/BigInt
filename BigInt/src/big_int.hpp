...
class BigInt {
  public:
    ...
    // friend functions
    friend std::ostream &
    operator<<(std::ostream &out, const BigInt &integer); // Outputting a BigInt
    friend BigInt operator"" _b(const char *s); // user defined literal: can use
                                                // as "BigInt num = 123_b;"
    friend BigInt operator+(const BigInt &left,
                            const BigInt &right); // add two bigints
    friend BigInt operator-(const BigInt &left,
                            const BigInt &right); // subtract two bigints
    friend BigInt operator*(const BigInt &left,
                            const BigInt &right); // multiply two bigints

    // static functions
    static int compare(const BigInt &left,
                       const BigInt &right); // -1 if left < right; 0 if left ==
  ...
  private:
    ...
    static BigInt halfAdd(const BigInt &left,
                          const BigInt &right); // add two non-negative bigints
    static BigInt
    halfSubtract(const BigInt &left,
                 const BigInt &right); // subtract smaller from bigger
    static BigInt
    multiplyNaive(const BigInt &left,
                  const BigInt &right); // grade-school multiplication
    static BigInt
    multiplyKaratsuba(const BigInt &left,
                      const BigInt &right); // Karatsuba multiplication
    static BigInt
    multiplyKaratsubaHelper(const BigInt &left,
                            const BigInt &right); // Karatsuba multiplication
