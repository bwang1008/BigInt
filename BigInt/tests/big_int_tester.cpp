...
bool test_mul_07() {
	BigInt x{"31415926535897932384626433832795028841971693993751058209749445923078164062862089986280348253421170679"};
	BigInt y{"27182818284590452353602874713526624977572470936999595749669676277240766303535945713821785251664274"};
	BigInt z = x * y;
	
	return z.str() == "853973422267356706546355086954657449503488853576511496187960113017922861115747838957625075000327286593478162070678673043735453603785848133094851462428844661315312664336578255202869537848950160622046";
}
...
TestCasesSummary runTestCases() {
	...
	for(std::map<std::string, functionPointer>::iterator it = namesToFunctions.begin(); it != namesToFunctions.end(); ++it) {
...
int main() {
	...
	if(summary.erroredFunctions.size() > 0) {
		std::cout << "First test case with compilation error: " << summary.erroredFunctions.front() << std::endl;
	}

	if(summary.failedFunctions.size() > 0) {
		std::cout << "First test case which failed: " << summary.failedFunctions.front() << std::endl;
	}
